...
#TODO from Super Wang: Forget about the following code and please use "service <ProfileName>-<ServerName>_was.init start" to start WAS
...
echo "As non-Root user [${WAS_SYS_USERNAME}] start the WAS server ....."
...
if ps ax | grep -v grep | grep "${WAS_PROFILE_ROOT}" | grep "${WAS_SERVER_NAME}" > /dev/null ; then
	...
	su - ${WAS_SYS_USERNAME} -c "${WAS_PROFILE_ROOT}/bin/startServer.sh ${WAS_SERVER_NAME}"
	if [ $? -ne 0 ]; then
...
echo "Sleeping 30 second to wait everything started."
