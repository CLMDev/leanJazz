    ...
    before(function(done) {
        client.post('/api/v1/topology/pools', newdata, function(err, res, body) {
            if (! err) {
                ...
                assert.equal(body.poolingMethod.type, 'basic', 'Expected:basic,Actual:' + body.poolingMethod);
                ...
                console.log(err);
                assert.equals(err, undefined, 'could not create test pool');
                done();
            }
        });
    });
        after(function(done) {
                ...
                if (remove == 'false') {
                        console.log('SimpleTopologyService Pool API v1.after:keeping test data');
                        done();
                }else {
                        client.del('/api/v1/topology/pools/' + newdata.name, function(err, res, body) {
                                console.log('SimpleTopologyService Pool API v1.after:removed test data');
                                done();
                        });
                }
        });
                ...
                client.post('/api/v1/topology/pools', invalidPoolData, function(err, res, body) {
                    ...
                    if (res.statusCode == 200) {
                        client.del('/api/v1/topology/pools/' + invalidPoolData.name, function(err, res, body) {
                            console.log('POST /api/v1/topology/pools: removed invalid test data');
                            done();
                        });
                    }else {
                        done();
